...
exports.useGafpriPaginationsAccountsReceivable = useGafpriPaginationsAccountsReceivable;
...
function useGafpriPaginationsAccountsReceivable(_ref) {
  var data = _ref.data;
  ...
  var filterByName = function filterByName(search) {
    if (data.states.items.data.items) {
      return data.states.items.data.items.filter(function (storage) {
        return storage.entity.name.toLowerCase().includes(search.toLowerCase());
      });
    }
    return null;
  };
