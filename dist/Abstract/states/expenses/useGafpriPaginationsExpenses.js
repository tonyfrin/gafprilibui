...
exports.useGafpriPaginationsExpenses = void 0;
...
var useGafpriPaginationsExpenses = exports.useGafpriPaginationsExpenses = function useGafpriPaginationsExpenses(_ref) {
  ...
  function sortByName(items, order) {
    if (items) {
      return items.slice().sort(function (a, b) {
        var comparison = a.expensesSupplier[0].name.localeCompare(b.expensesSupplier[0].name, undefined, {
          sensitivity: 'base'
        });
  ...
  var filterByName = function filterByName(search) {
    if (useData.states.items && useData.states.items.data.items) {
      return useData.states.items.data.items.filter(function (item) {
        return item.expensesSupplier[0].name.toLowerCase().includes(search.toLowerCase());
      });
    }
    return null;
  };
