...
exports.useGafpriPaginationsIncome = void 0;
...
var useGafpriPaginationsIncome = exports.useGafpriPaginationsIncome = function useGafpriPaginationsIncome(_ref) {
  ...
  function sortByName(items, order) {
    if (items) {
      return items.slice().sort(function (a, b) {
        var comparison = a.incomeCustomer[0].name.localeCompare(b.incomeCustomer[0].name, undefined, {
          sensitivity: 'base'
        });
  ...
  var filterByName = function filterByName(search) {
    if (useData.states.items && useData.states.items.data.items) {
      return useData.states.items.data.items.filter(function (item) {
        return item.incomeCustomer[0].name.toLowerCase().includes(search.toLowerCase());
      });
    }
    return null;
  };
      ...
      return items.slice().sort(function (a, b) {
        ...
        switch (search) {
          ...
          case 'total':
            ...
            break;
          case 'name':
            aValue = a.incomeCustomer[0].name;
            bValue = b.incomeCustomer[0].name;
            break;
          default:
    ...
    if (useData.states.items.data.items) {
      return useData.states.items.data.items.filter(function (item) {
        ...
        switch (searchType) {
          ...
          case 'total':
          case 'name':
            valueToCheck = item.incomeCustomer[0].name;
            break;
          case 'status':
