...
exports.useGafpriPaginationsOrderReturn = void 0;
...
var useGafpriPaginationsOrderReturn = exports.useGafpriPaginationsOrderReturn = function useGafpriPaginationsOrderReturn(_ref) {
  ...
  function sortByName(items, order) {
    if (items) {
      return items.slice().sort(function (a, b) {
        var comparison = a.order.orderCustomer[0].name.localeCompare(b.order.orderCustomer[0].name, undefined, {
          sensitivity: 'base'
        });
  ...
  var filterByName = function filterByName(search) {
    if (useData.states.items && useData.states.items.data.items) {
      return useData.states.items.data.items.filter(function (item) {
        return item.order.orderCustomer[0].name.toLowerCase().includes(search.toLowerCase());
      });
    }
    return null;
  };
  ...
  var changeSearchBy = function changeSearchBy(options) {
    ...
    var value = options !== null && options !== void 0 && options.value && (options.value === 'name' || options.value === 'postsId' || options.value === 'total') ? options.value : 'name';
    var newDefaulValue = {
      label: label,
      value: value
    };
