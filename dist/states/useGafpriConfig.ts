...
import { validationSelect, removeClass, addClass } from '../helpers';
    ...
    type Actions = {
        onConfig: () => void;
        nextCurrency: () => void;
        currencyChange: (newValue: SingleValue<{value: string; label: string;}>) => void;
    ...
    function useGafpriConfig() {
         ...
         * 
         */
            const onConfig = () => dispatch({
                type: actionTypes.isConfig,
            });

            const onSites = () => dispatch({
                type: actionTypes.isSites,
            });

            //setear los valores de la Currencies
                const setCurrenciesName = (value: string): void => dispatch({
                    type: actionTypes.currencyName,
                    payload: value
                });

                const setCurrenciesSymbol = (value: string): void => dispatch({
                    type: actionTypes.currencySymbol,
                    payload: value
                });

                const setCurrenciesDefault = (value: CurrencyDefault): void => dispatch({
                    type: actionTypes.currenciesDefault,
                    payload: value
                });

                const setValidationCurrency = (value: boolean): void => dispatch({
                    type: actionTypes.validationCurrency,
                    payload: value
                });

                const setButtonCurrency = (value: boolean): void => dispatch({
            ...
            //Funciones de validations
                const validationCurrencyValue = (value: string): void => {
                    const validation: boolean = validationSelect(value, 'currencyName', 'gs-input-alert');
