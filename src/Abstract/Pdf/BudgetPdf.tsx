...
export const BudgetPdf: React.FC<BudgetPdfProps> = ({
  budget,
  logo,
  siteOptions,
}) => {
  const state =
    (budget.budgetCustomer[0].address[0]?.country &&
      budget.budgetCustomer[0].address[0]?.state &&
      StatesCountries[0][budget.budgetCustomer[0].address[0].country]?.[0]?.[
        budget.budgetCustomer[0].address[0].state
      ]) ||
    '';
  ...
  const country =
    (budget.budgetCustomer[0].address[0]?.country &&
      Countries[0][budget.budgetCustomer[0].address[0].country]) ||
    '';
  ...
  const customer = budget.budgetCustomer[0] || {};
    ...
    <Document>
      <Page style={styles.document} size="A4">
        ...
        <View style={styles.TitleContainer}>
          <Text>PRESUPUESTO {budget.postsId}</Text>
        </View>
          ...
          <View style={styles.orderData}>
            <Text style={styles.line}>
              <Text>
                ...
                {formatDate(`${budget.posts.createdAt}`)}
              </Text>
            </Text>
            <Text style={styles.line}>
              <Text>
                <Text style={styles.bold}>Pedido:</Text> {budget.postsId}
              </Text>
            </Text>
            <Text style={styles.line}>
              <Text>
        ...
        <View style={styles.finalSection}>
          <View style={styles.noteSection}>
            <Text style={styles.line}>
              <Text>
                <Text style={styles.bold}>Nota:</Text> {budget.note}
              </Text>
            </Text>
          </View>
          <View style={styles.totalSection}>
            ...
            <Text style={styles.line}>
              <Text>
                <Text style={styles.bold}>Total del Presupuesto:</Text>{' '}
                {decimalFormatPriceConverter(
                  budget.total,
                  dig,
                  currencySymbol,
        ...
        <View style={styles.footer}>
          {siteOptions.footerBudget.map((item) => (
            <Text>{item}</Text>
          ))}
        </View>
      </Page>
