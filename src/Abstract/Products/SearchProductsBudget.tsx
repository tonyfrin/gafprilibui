...
const searchProductsBudgetStylesContainer = () => css`
  max-width: 1150px;
  margin: auto;
...
export const SearchProductsBudget = ({
  use,
  useBudget,
  sitesOptions,
  ...
  listProps,
}: SearchProductsBudgetProps): JSX.Element => {
    ...
    return (
      <div className={css(optionsButtonMainContainerStyle)}>
        <Button
          title="Seleccionar"
          buttonProps={{
            onClick: () => useBudget.pages.actions.selectproduct(id),
          }}
          styles={{
            fontSize: '10px',
          }}
  ...
  return (
    <>
      <div className={cx(searchProductsBudgetStylesContainer())}>
        <Header {...headerProps} />
        <Error {...errorProps} />
        <List
          title="Productos"
